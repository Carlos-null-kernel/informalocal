...
var arquivos = ["arq.json"];
function loadItems() {
  fetch('arq.json')
    .then(response => response.json())
    .then(data => {
      items = data; // armazena todos os itens JSON no array "items"
      displayItems(); // exibe os primeiros 10 itens
    });
}

function displayItems() {
  const container = document.getElementById('item-containerp');
  ...
  for (let i = displayedItems; i < displayedItems + 12 && i < items.length; i++) {
    const item = items[i];
    const itemHTML = `<li><a href="${item.Link}">${item.Titulo}</a></li>`;
...
function buscar() {
  ...
  Promise.all(arquivos.map(arquivo =>
    ...
    const resultadosFiltrados = items.filter(item => {
      ...
      const descricao = removerAcentos(item.Link.toLowerCase());
    ...
    const resultadoDiv = document.getElementById("item-containerp");
    resultadoDiv.innerHTML = "";
    if (resultadosFiltrados.length > 0) {
      resultadosFiltrados.forEach(item => {
        resultadoDiv.innerHTML += `<li><a href="${item.Link}">${item.Titulo}</a></li>`;
      });
    } else {
